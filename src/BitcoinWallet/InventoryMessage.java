            ...
            try {
                if (type == Parameters.INV_TX) {
                    if (Parameters.wallet.isNewTransaction(hash)) {
                        //
                        // This is a new transaction, so request it from the peer
                        //
                        PeerRequest request = new PeerRequest(hash, Parameters.INV_TX, peer);
                    ...
                    if (Parameters.wallet.isNewBlock(hash) ||
                        ...
                        //
                        // This is a new block or we are down-level, so get the block from the peer
                        //
                        PeerRequest request = new PeerRequest(hash, Parameters.INV_FILTERED_BLOCK, peer);
                        synchronized(Parameters.lock) {
                            if (!Parameters.pendingRequests.contains(request) &&
                                            !Parameters.processedRequests.contains(request))
                                Parameters.pendingRequests.add(request);
